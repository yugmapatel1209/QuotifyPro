...
@section('top-content')
    <div class="col-lg-10">
        <h2>Earth Enterprise - Quotations</h2>
    </div>
@endsection
        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <form action="{!! route('earth.create') !!}" class="device-content-center">
                        <button class="btn btn-sm btn-primary pull-right m-t-n-xs"><strong><i class="fa fa-plus"></i> Add
                ...
                <div class="ibox-content">
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="ascend-tables"
                        style="width:100%">
                        <thead>
                            <tr>
            ...
            columns: [{
                ...
                },
                {
                    ...
                    "render": function(data, type, row) {
                        return row.is_active == 1 ? '<span class="label label-primary">Open</span>' :
                    ...
                    "data": "ExerciseId",
                    "render": function(data, type, row) {
                        var button = '<a href="{{ url('earth') }}' + '/' + row['id'] +
                            '/edit" class="product-table-info" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil"></i></a>';
                        button += '<a href="{{ url('earth') }}' + '/' + row['id'] +
                            '" class="product-table-info" data-toggle="tooltip" title="View" data-original-title="View"><i class="fa fa-eye"></i></a>';
                        button += '<a onclick="deleteData(\'' + row['id'] +
                            '\')"  class="product-table-danger" data-toggle="tooltip" title="Delete"><i class="fa fa-trash"></i></a>';
                        return button;
                    }
                }
            ],
                ...
                success: function(response) {
                    ...
                    if (data_lan > 0) {
                        ...
                        for (i = 0; i < data_lan; i++) {
                            var option = $('<option></option>').text(data[i].Name).val(data[i].Name)
                                .attr('data-id', data[i].SubjectId);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (data_lan > 0) {
                            ...
                            for (i = 0; i < data_lan; i++) {
                                var option = $('<option></option>').text(data[i].Name).val(data[i].Name)
                                    .attr('data-id', data[i].ScenarioId);
